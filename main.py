...
def within_trading_hours():
    ...
    # Best window for this bot: London + NY overlap
    # Monday-Friday 3AM–11AM ET only
    if now_et.weekday() in [0, 1, 2, 3, 4]:
        return 3 <= now_et.hour < 11
...
def place_live_order(pair, direction, entry_price):
    ...
    broker_stop_price = calculate_broker_stop_price(pair, direction, entry_price)
...
def open_trade(pair, direction, entry_price, score):
    ...
    active_trades[pair] = {
        ...
        "broker_stop_price": live_result["broker_stop_price"]
    }
...
Broker SL: {live_result["broker_stop_price"]}
...
Broker SL: {trade.get('broker_stop_price')}
